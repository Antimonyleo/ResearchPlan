...
class MalformedStateTests(unittest.TestCase):
    ...
    def test_nested_reference_and_review_types_return_errors(self):
        cases = (
            ...
            ("campaign.work_units.0.external_action_ids", False,
             lambda state: state["campaign"].update(work_units=[{"id": "unit-1", "external_action_ids": [{}]}])),
            ("campaign.work_units.0.approval_ids", False,
             lambda state: state["campaign"].update(work_units=[{"id": "unit-1", "approval_ids": [{}]}])),
