...
3. **Host primitives.** Anthropic's runs used private host capabilities for delegation, compute, gating, and notification (`host.delegate`, `host.compute`, `submit_gate`, `wait_for_notification`). A portable skill cannot assume these exist, so ResCamp specifies required operations in the plan but does not supply a scheduler.
4. **Content density.** The protocol prompt is about 16,000 words of specific, hard-won domain knowledge for one field. ResCamp's `SKILL.md` is about 2,000 words and contains none of it. It compiles a structure that a domain expert must fill; it does not know any domain.
...
7. **Material in-run amendments.** The paper iterated the protocol during test campaigns and froze it before the reported runs. It allowed target-specific decisions inside that protocol, but does not describe versioning, re-review, or approval for changing the governing scientific plan after a reported campaign began. ResCamp's controlled-amendment procedure is an extrapolation.
